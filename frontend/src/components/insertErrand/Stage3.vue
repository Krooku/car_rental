<template>
  ...
  >
    ...
    />
    <input-label
      v-model="errandObj.kilometers.towing"
      :type="2"
      label-text="Holowanie(KM)"
    />
    <input-label
      v-model="errandObj.destinationPlace"
      :type="0"
      :maxlength="64"
      label-text="Docelowe miejsce holowania"
    />
    <div class="date-box">
    ...
    <select v-model="errandObj.handed">
      <option>serwis ASO</option>
      <option>inny warsztat</option>
      <option>dom klienta</option>
      <option>inne</option>
    </select>
    <input-label
    ...
    <div class="flex_box answer">
      <div
        data-text="Wróc(Dojazd)"
        class="btn btn--winona"
        @click="nextStage(true, false)"
      >
        <span>Wróc(Dojazd)</span>
      </div>
      <div
        data-text="Dalej(Powrót)"
        class="btn btn--winona"
        @click="nextStage()"
      >
        <span>Dalej(Powrót)</span>
      </div>
    </div>
  </form>
</template>
...
export default {
  ...
  data () {
    return {
      ...
      loaded: true,
      error: null,
      properties: ['kilometers', 'destinationPlace', 'journey', 'handed', 'description']
    }
  },
  mounted () {
    if (this.errand) {
      ...
      if (this.errandObj.journey.towing) {
        const newDate = new Date(this.errandObj.journey.towing)
        if (this.errandObj.journey.towing instanceof Date) {
          this.fullDate.date = this.formatDate(0, newDate)
        } else {
          this.fullDate.date = this.errandObj.journey.towing
        }
  ...
  methods: {
    ...
    async nextStage (next = true, add = true) {
      ...
      const oldDate = this.errandObj.journey.towing
      this.errandObj.journey.towing = date
      if (!this.getChanges(this.startErrandObject, this.errandObj)) {
        this.errandObj.journey.towing = oldDate
      ...
      this.loaded = false
      await api.updateTowingErrandTowing({
        errandId: this.errandId,
        kilometers: this.errandObj.kilometers.towing,
        date: date,
        destinationPlace: this.errandObj.destinationPlace,
        handed: this.errandObj.handed,
